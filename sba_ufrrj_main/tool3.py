...
...
def calcular(projeto, tabela, campo):
    planilha_angulos = pd.read_csv(projeto+'/ferramenta3_angulos.csv', sep=';')
    series = planilha_angulos["serie"].unique().tolist()
    series.sort()
    conjuntos = planilha_angulos["conjunto"].unique().tolist()
    conjuntos.sort()
    alvos = planilha_angulos["alvo"].unique().tolist()
    alvos.sort()
    ...
    for s in series:
        for c in conjuntos:
            subplanilha_angulos = planilha_angulos.loc[planilha_angulos['serie'] == s]
            subplanilha_angulos = subplanilha_angulos.loc[subplanilha_angulos['conjunto'] == c]
            angulo_reducao = subplanilha_angulos.iloc[0]['angulo horizontal decimal']
            for i, row in subplanilha_angulos.iterrows():
                reducao = round(float(row['angulo horizontal decimal']) - angulo_reducao, 8)
                if reducao < 0:
                    reducao*=-1
                reducoes.append(reducao)
    planilha_angulos['reducao'] = reducoes

    medias_reducoes = []
    for s in series:
        for a in alvos:
            subplanilha_angulos = planilha_angulos.loc[planilha_angulos['serie'] == s]
            subplanilha_angulos = subplanilha_angulos.loc[subplanilha_angulos['alvo'] == a]
            medias_reducoes.append(subplanilha_angulos['reducao'].mean())
    print(medias_reducoes)
    """
    medias_reducoes = [
        0.00000,
        49.30562,
    ...
    for s in series:
        ...
        for c in conjuntos:
            ...
            j = 0
            for i, row in subplanilha_angulos.iterrows():
                diferenca_t = float(row['reducao']) - medias_reducoes[cont+j]
                print(f'''{float(row['reducao'])}-{medias_reducoes[cont+j]}''')
                print(diferenca_t)
                diferenca = round(diferenca_t * 3600, 8)
                diferencas.append(diferenca)
                j+=1
        cont+=5
    planilha_angulos['diferenca'] = diferencas
    ...
    for s in series:
        for c in conjuntos:
            subplanilha_angulos = planilha_angulos.loc[planilha_angulos['serie'] == s]
            subplanilha_angulos = subplanilha_angulos.loc[subplanilha_angulos['conjunto'] == c]
            medias_diferencas.append(round(subplanilha_angulos['diferenca'].mean(), 8))
    ...
    for s in series:
        ...
        for c in conjuntos:
            ...
            cont+=1
    planilha_angulos['desvio'] = desvios
    ...
    for s in series:
        for c in conjuntos:
            subplanilha_angulos = planilha_angulos.loc[planilha_angulos['serie'] == s]
            subplanilha_angulos = subplanilha_angulos.loc[subplanilha_angulos['conjunto'] == c]
            soma_desvios.append(round(subplanilha_angulos['desvio'].sum(), 8)) 

    desvios_quadrados = []
    for d in desvios:
        desvios_quadrados.append(round(d*d,8))
    planilha_angulos['desvio quadrado'] = desvios_quadrados
    ...
    for s in series:
        for c in conjuntos:
            subplanilha_angulos = planilha_angulos.loc[planilha_angulos['serie'] == s]
            subplanilha_angulos = subplanilha_angulos.loc[subplanilha_angulos['conjunto'] == c]
            soma_desvios_quadrados.append(subplanilha_angulos['desvio quadrado'].sum()) 
    ...
    with open(projeto+'/config.ajobs', 'r') as outfile:
        data = json.load(outfile)
        data['ferramenta3'][0]['desvio_padrao_exp_geral'] = str(desvio_padrao_exp_geral)
    with open(projeto+'/config.ajobs', 'w') as outfile:
        json.dump(data, outfile)

    campo.setText(str(desvio_padrao_exp_geral))

    planilha_angulos.to_csv(projeto+'/ferramenta3_resultados.csv', index=False, sep=';')
    with open(projeto+'/ferramenta3_resultados.csv', newline='') as csv_file:
...
def lerAngulos(caminho, projeto, tabela, separador):
    planilha_angulos = pd.read_csv(caminho, sep=';')
    print(planilha_angulos)

    #calculando azimutes
    ang_sex = []
    ang_dec = []
    for i, row in planilha_angulos.iterrows():
        #ang_pd = float(row['pd'])#Conversoes.grsex_para_grdec(row['pd'])
        ang_pd = Conversoes.grsex_para_grdec(row['pd'])
        #ang_pd+=10
        if ang_pd >= 360:
            ang_pd-=360

        #ang_pi = float(row['pi'])#Conversoes.grsex_para_grdec(row['pi'])
        ang_pi = Conversoes.grsex_para_grdec(row['pi'])
        #ang_pi+=10
        if ang_pi >= 360:
            ang_pi-=360
        ...
        ang = 0.0
        if(ang_pi < 180):
            ang = (ang_pd + ang_pi + 180)/2
        else:
            ang = (ang_pd + ang_pi - 180)/2
        ang_dec.append(round(ang, 8))
        ang_sex.append(Conversoes.grdec_para_grsex(ang))
    planilha_angulos['angulo horizontal'] = ang_sex
    planilha_angulos['angulo horizontal decimal'] = ang_dec
    planilha_angulos.to_csv(projeto+'/ferramenta3_angulos.csv', index=False, sep=';')

    with open(projeto+'/ferramenta3_angulos.csv', newline='') as csv_file:
        tabela.setRowCount(0)
